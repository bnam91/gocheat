/* ⚠️★2026-09-03 이 파일의 «옛 배포 금지 경고는 무효»다 — 지웠다.
 *   그 경고는 「레포본이 라이브보다 뒤처졌으니 올리지 마라」였는데, 실측하니 반대였다:
 *   레포와 라이브의 «실행 코드가 sha256 까지 같다»(주석만 달랐다).
 *   ★그 거짓 경고가 실제로 사람을 멈춰 세웠다(2026-09-03, 지디가 배포를 중단했다).
 *   ⇒ 거짓 경고는 «경고가 없는 것보다 나쁘다» — 진짜 경고와 구분이 안 되기 때문이다.
 *     ⛔여기에 경고를 적을 땐 «언제·무엇으로 확인했는지»를 같이 적어라.
 *
 * ⛔★★남아 있는 «진짜» 결손 — 2026-09-03 실측으로 확인했다(추측 아님):
 *   이 파일은 «제품별 세션 칸»(_lib/sessions.js 의 issueSession)을 부르지 않는다.
 *   56·58·99행이 sessionToken «한 칸»만 쓴다. 2026-08-25 에 들어갔던 구조가 회귀한 상태다.
 *   ★실측 증거: 새 계정으로 app:'goditor' 로그인 1회 → users.sessions[] 가 «생기지 않았다».
 *     sessionToken 한 칸만 갱신됐다.
 *   ★증상(2026-09-03 실제 발생): 같은 계정으로 홈페이지에 로그인하면 «앱·확장의 로그인이 풀린다».
 *     그날 담당자가 QA 로 웹 로그인을 한 번 했더니 대표 계정의 브라우저 세션이 그 자리에서 끊겼다.
 *   ⇒ 복구 패치: 지디 스킬 handoff/unitA-server-0.8.6/patches/login.js.restore-sessions.patch
 *   ⚠️적용은 «현빈 승인» 사항이다 — 로그인 핵심 경로이고, 앱별 번들(tiers) 작업과 같은 자리를 만진다.
