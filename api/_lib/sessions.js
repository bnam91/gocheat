...
// 앱당 몇 개까지 살려둘까. 1 = 한 앱은 한 곳에서만. 2 = 같은 앱을 두 곳까지 동시에.
// ★계정 공유 억제와 직결되는 값이라 «정책»이다 — 바꾸려면 현빈 승인.
//
// ★★2026-09-03 현빈 승인으로 1 → 2.
//   이유: 대표가 맥과 사무실 윈도우를 «둘 다» 쓰는데 1 이면 뒤에 켠 쪽만 살아남아,
//         한쪽에서 로그인할 때마다 다른 쪽이 그 자리에서 풀렸다.
//   ⚠️대가: 계정 하나를 «두 사람이» 나눠 쓸 수 있게 된다. 지금은 베타라 전원 무료여서
//     잃는 게 없지만, ★유료 판매를 시작하면 그게 곧 매출 손실이다.
//     ⇒ 유료 전환 시 이 값을 «다시 검토»할 것. (되돌리는 건 이 한 줄이다)
//   ★앱 «종류»마다 따로 센다 — goditor 2 + godiv 2 + web 2 … 전체 상한은 MAX_TOTAL 이 잡는다.
const MAX_PER_APP = 2;
