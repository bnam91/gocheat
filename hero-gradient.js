...
(function () {
  ...
  /* ★2026-09-05 밤 — 조금 빠르게(현빈: 「속도가 좀 느린 것 같다, 아주 쪼금만 올려보자」).
       14~22초(평균 18) → 11.5~17.5초(평균 14.5). 약 «1.24배»다.
     ⚠️여기서 더 올리면 «흐름»이 아니라 «깜빡임»이 된다 — 은은함이 목적이라 배수로 안 올렸다.
     ★줄마다 어긋나는 delay 도 «같은 비율»로 줄인다(2600 → 2100). 안 줄이면 빨라진 주기에
       비해 어긋남이 커져서 세 줄이 물결이 아니라 «따로» 논다. */
  lines.forEach(function (el, i) {
    el.animate(randPath(), {
      duration: 11500 + Math.random() * 6000,
      iterations: Infinity,
      easing: 'ease-in-out',
      delay: -i * 2100        // 줄마다 어긋나게 — 세 줄이 물결처럼 이어진다
    });
  });
})();
